...
export const ReferralDashboard: React.FC = () => {
  ...
  const { data: referralData, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data: referralCode } = await apiClient.get(`/api/admin/referral_codes`, { params: { user_id: user.id } });

      // Get referrals made by this user
      const { data: referrals } = await apiClient.get('/api/admin/referrals')
        `)
        .eq('referrer_id', user.id)
        .order('created_at', { ascending: false });

      // Get total rewards earned
      const { data: rewards } = await apiClient.get(`/api/admin/referral_rewards`, { params: { user_id: user.id } });
