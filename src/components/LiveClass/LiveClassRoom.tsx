  ...
  useEffect(() => {
    const fetchSessionDetails = async () => {
      ...
      try {
        ...
        const { data } = await apiClient.get(`/api/live-sessions/${actualSessionId}`);
        ...
        if (data) {
          setSession({
            ...
            scheduledStartTime: data.scheduled_start_time || data.scheduledStartTime,
            scheduledEndTime: data.scheduled_end_time || data.scheduledEndTime,
            ...
            recordedUrl: data.recorded_url || data.recordedUrl,
            chatEnabled: data.chat_enabled ?? data.chatEnabled ?? true,
            isActive: data.is_active ?? data.isActive ?? false,
            accessLevel: (data.access_level || data.accessLevel || "free") as "free" | "paid" | "subscription"
          });
        }
      } catch (error) {
