...
export const PurchaseButton: React.FC<PurchaseButtonProps> = ({
  ...
  const recordCouponUsage = async (couponCode: string, orderId: string) => {
    try {
      const { data: coupon } = await apiClient.get(`/api/admin/coupons`, { params: { code: couponCode } });

      if (coupon) {
        // Record usage
        await apiClient.post('/api/admin/coupon_usage', {
            coupon_id: coupon.id,
            user_id: (await ({ data: { user: auth.currentUser } })).data.user?.id,
            order_id: orderId,
            discount_amount: discount
          });

        // Update usage count - increment by 1
        await apiClient.put(`/api/admin/coupons/${coupon.id}`, { used_count: coupon.used_count + 1 });
      }
    } catch (error) {
  ...
  const handlePurchase = async () => {
    try {
      ...
      // Get current user
      const { data: { user }, error: userError } = await ({ data: { user: auth.currentUser } });
      if (userError || !user) {
        toast({
