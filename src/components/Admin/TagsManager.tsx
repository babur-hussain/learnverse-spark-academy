...
const TagsManager = () => {
  ...
  const createTag = useMutation({
    mutationFn: async (tag: { name: string; type: string }) => {
      const { data, error } = await apiClient.post('/api/admin/tags', [{ name: tag.name, type: tag.type }]);
  ...
  const updateTag = useMutation({
    mutationFn: async (tag: Tag) => {
      const { data, error } = await apiClient.put(`/api/admin/tags/${tag.id}`, { name: tag.name, type: tag.type });
  ...
  const deleteTag = useMutation({
    mutationFn: async (id: string) => {
      const { error } = await apiClient.delete(`/api/admin/tags/${id}`);
