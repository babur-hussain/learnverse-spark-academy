...
export const AdminAnalytics = () => {
  ...
  const { data: analyticsData, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { count: totalUsers } = await apiClient.get('/api/admin/profiles');
      ...
      const { count: totalSubjects } = await apiClient.get('/api/admin/subjects');

      // Get recent user registrations (last 30 days)
      const thirtyDaysAgo = new Date();
      thirtyDaysAgo.setDate(thirtyDaysAgo.getDate() - 30);

      const { data: recentUsers } = await apiClient.get('/api/admin/profiles', {
        params: { order_by: 'created_at', sort: 'asc' }
      });

      // Get course enrollment data
      const { data: courseEnrollments } = await apiClient.get('/api/admin/user_courses');

      // Get live sessions data
      const { data: liveSessions } = await apiClient.get('/api/admin/live_sessions');

      // Get forum activity
      const { count: forumThreads } = await apiClient.get('/api/admin/forum_threads');

      const { count: forumPosts } = await apiClient.get('/api/admin/forum_posts');
