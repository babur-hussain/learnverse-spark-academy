...
const NotesManager: React.FC<NotesManagerProps> = ({ 
  ...
  const { data: chapters = [] } = useQuery({
    ...
    queryFn: async () => {
      let query = apiClient.get('/api/admin/chapters');
  ...
  const { data: tags = [] } = useQuery({
    queryKey: ['tags'],
    queryFn: async () => {
      const { data, error } = await apiClient.get('/api/admin/tags');
  ...
  const { data: noteTags = [] } = useQuery({
    queryKey: ['note_tags'],
    queryFn: async () => {
      const { data, error } = await apiClient.get('/api/admin/note_tags');
  ...
  const { data: notes = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      let query = apiClient.get('/api/admin/note_files');
  ...
  const createNote = useMutation({
    mutationFn: async (note: {
      ...
      const { data: uploadData, error: uploadError } = await uploadFileToS3(note.file, filePath);

      if (uploadError) {
        throw uploadError;
      }

      const { data: urlData } = // S3 URL managed by backend

      const { data, error } = await apiClient.post('/api/admin/note_files', [{
    ...
    onSuccess: async (data) => {
      ...
      if (selectedTags.length > 0) {
        ...
        const { error: noteTagsError } = await apiClient.post('/api/admin/note_tags', noteTagInserts);
  ...
  const updateNote = useMutation({
    mutationFn: async (note: NoteFile) => {
      const { data, error } = await apiClient /* note_files */
        .update({
          title: note.title,
    ...
    onSuccess: async (data) => {
      ...
      const { error: deleteError } = await apiClient.delete(`/api/admin/note_tags/${noteId}`);
  ...
  const deleteNote = useMutation({
    mutationFn: async (id: string) => {
      const { error } = await apiClient.delete(`/api/admin/note_files/${id}`);
  ...
  const handleDownloadNote = async (note: NoteFile) => {
    try {
      ...
      await apiClient.functions.invoke('increment_note_download_count', {
