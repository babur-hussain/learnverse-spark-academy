...
export const CollegeResourceDialog: React.FC<CollegeResourceDialogProps> = ({
  ...
  const { data: chapters = [] } = useQuery({
    ...
    queryFn: async () => {
      if (!subjectId) return [];
      const { data, error } = await apiClient.get(`/api/admin/chapters`, { params: { subject_id: subjectId } });
  ...
  const uploadFile = async (file: File): Promise<string> => {
    ...
    try {
      ...
      const cleanup = simulateProgress();
              ...
              // Use S3 upload via presigned URL
        const bucketName = 'collegecontent';

      const publicUrl = await uploadFileToS3(file, `${bucketName}/${filePath}`);
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    try {
      ...
      const { error } = resource 
        ? await apiClient.put(`/api/admin/subject_resources/${resource.id}`, resourceData)
        : await apiClient.post('/api/admin/subject_resources', resourceData);
