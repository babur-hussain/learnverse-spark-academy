  ...
  const handleFileUpload = async (files: FileList) => {
    ...
    for (const uploadFile of newFiles) {
      try {
        ...
        const publicUrl = await uploadFileToS3(uploadFile.file, filePath);
        ...
        const { data: existingResource } = await apiClient.get(`/api/admin/subject_resources`, { params: { subject_id: subjectId, file_name: uploadFile.file.name } });

        if (existingResource) {
          // Update existing resource
          const { error: dbError } = await apiClient /* subject_resources */
            .update({
              file_url: publicUrl,
          ...
          // Create new resource
          const { error: dbError } = await apiClient.post('/api/admin/subject_resources', {
  ...
  const handleFolderUpload = async (files: FileList) => {
    ...
    for (const uploadFile of newFiles) {
      try {
        ...
        const { error: uploadError } = await uploadFileToS3(uploadFile.file, filePath);
