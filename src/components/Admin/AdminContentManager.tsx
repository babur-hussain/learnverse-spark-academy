...
export const AdminContentManager = () => {
  ...
  const { data: contentStats, isLoading: isLoadingStats } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { count: totalCourses } = await apiClient.get('/api/admin/courses');

      // Get total videos count
      const { count: totalVideos } = await apiClient.get('/api/admin/videos');

      // Get total resources count
      const { count: totalResources } = await apiClient.get('/api/admin/subject_resources');
  ...
  const { data: courses = [], isLoading: isLoadingCourses } = useQuery({
    ...
    queryFn: async () => {
      const { data: coursesData, error } = await apiClient.get('/api/admin/courses');

      if (error) throw error;

      // Get instructor names
      const instructorIds = coursesData?.map(c => c.instructor_id).filter(Boolean) || [];
      const { data: profiles } = await apiClient.get('/api/admin/profiles');
      ...
      const { data: enrollments } = await apiClient.get('/api/admin/user_courses');
  ...
  const { data: videos = [], isLoading: isLoadingVideos } = useQuery({
    ...
    queryFn: async () => {
      const { data: videosData, error } = await apiClient.get('/api/admin/videos');

      if (error) throw error;

      // Get course titles
      const courseIds = videosData?.map(v => v.course_id).filter(Boolean) || [];
      const { data: coursesData } = await apiClient.get('/api/admin/courses');
  ...
  const { data: resources = [], isLoading: isLoadingResources } = useQuery({
    ...
    queryFn: async () => {
      const { data: resourcesData, error } = await apiClient.get('/api/admin/subject_resources');
