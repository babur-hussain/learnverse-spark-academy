...
const AccessManager = () => {
  ...
  const { data: notes = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await apiClient.get('/api/admin/note_files');
  ...
  const { data: noteAccess = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await apiClient.get('/api/admin/note_access')
        `)
  ...
  const createNoteAccess = useMutation({
    mutationFn: async (access: {
      ...
      const { data, error } = await apiClient.post('/api/admin/note_access', [{
  ...
  const deleteNoteAccess = useMutation({
    mutationFn: async (id: string) => {
      const { error } = await apiClient.delete(`/api/admin/note_access/${id}`);
