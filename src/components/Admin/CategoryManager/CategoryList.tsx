...
export function CategoryList() {
  ...
  const toggleStatus = async (id: string, field: 'is_active' | 'is_public', value: boolean) => {
    const { error } = await apiClient.put(`/api/admin/categories/${id}`, { [field]: value });
  ...
  const updateOrder = async (id: string, direction: 'up' | 'down') => {
    ...
    const { error } = await apiClient.put(`/api/admin/categories/${id}`, { order_index: categories[newIndex].order_index });

    if (!error) {
      await apiClient.put(`/api/admin/categories/${categories[newIndex].id}`, { order_index: categories[currentIndex].order_index });
  ...
  const handleDelete = async (id: string) => {
    const { error } = await apiClient.delete(`/api/admin/categories/${id}`);
