...
export function FeaturedCategoriesList() {
  ...
  const toggleStatus = async (id: string, value: boolean) => {
    const { error } = await apiClient.put(`/api/admin/featured_categories/${id}`, { is_active: value });
  ...
  const updateOrder = async (id: string, direction: 'up' | 'down') => {
    ...
    const { error } = await apiClient.put(`/api/admin/featured_categories/${id}`, { order_index: featuredCategories[newIndex].order_index });
  ...
  const handleDelete = async (id: string) => {
    const { error } = await apiClient.delete(`/api/admin/featured_categories/${id}`);
