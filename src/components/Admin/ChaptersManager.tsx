...
const ChaptersManager: React.FC<ChaptersManagerProps> = ({ 
  ...
  const createChapter = useMutation({
    mutationFn: async (chapter: {
      ...
      const { data: existingChapters } = await apiClient.get(`/api/admin/chapters`, { params: { subject_id: chapter.subject_id } });
      
      const nextOrderIndex = existingChapters && existingChapters.length > 0
        ? existingChapters[0].order_index + 1
        : 0;
      
      const { data, error } = await apiClient.post('/api/admin/chapters', [{
          title: chapter.title,
  ...
  const updateChapter = useMutation({
    mutationFn: async (chapter: Chapter) => {
      const { data, error } = await apiClient /* chapters */
        .update({
          title: chapter.title,
  ...
  const deleteChapter = useMutation({
    mutationFn: async (id: string) => {
      const { error } = await apiClient.delete(`/api/admin/chapters/${id}`);
  ...
  const moveChapter = useMutation({
    mutationFn: async ({ chapterId, direction }: { chapterId: string, direction: 'up' | 'down' }) => {
      ...
      // Swap order indices
      const { error: error1 } = await apiClient.put(`/api/admin/chapters/${currentChapter.id}`, { order_index: newOrderIndex });
      
      if (error1) throw error1;
      
      const { error: error2 } = await apiClient.put(`/api/admin/chapters/${targetChapter.id}`, { order_index: currentChapter.order_index });
