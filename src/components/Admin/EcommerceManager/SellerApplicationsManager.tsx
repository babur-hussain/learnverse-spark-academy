...
export function SellerApplicationsManager() {
  ...
  const { data: applications, isLoading } = useQuery({
    ...
    queryFn: async () => {
      let query = apiClient.get('/api/admin/seller_applications');
  ...
  const updateStatusMutation = useMutation({
    mutationFn: async ({ id, status, reason }: { id: string; status: string; reason?: string }) => {
      const { error } = await apiClient /* seller_applications */
        .update({ 
          status, 
      ...
      if (status === 'approved') {
        ...
        if (application) {
          const { error: roleError } = await apiClient.post('/api/admin/user_roles', { 
