...
export function InventoryManager() {
  ...
  const { data: products } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await apiClient.get(`/api/admin/products`, { params: { is_active: true } });
  ...
  const { data: transactions, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await apiClient.get('/api/admin/inventory_transactions', { params: { sort: '-created_at', limit: 50 } });
