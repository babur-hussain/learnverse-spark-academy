...
export function CourseSubjectsDialog({ 
  ...
  const { data: courseSubjects, isLoading: isLoadingCourseSubjects } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await apiClient.get('/api/admin/course_subjects', { params: { course_id: courseId, sort: 'order_index' } });
  ...
  const { data: availableSubjects, isLoading: isLoadingSubjects } = useQuery({
    ...
    queryFn: async () => {
      const { data: existingSubjectIds } = await apiClient.get(`/api/admin/course_subjects`, { params: { course_id: courseId } });
  ...
  const addSubjectsMutation = useMutation({
    mutationFn: async (subjectIds: string[]) => {
      ...
      const { error } = await apiClient.post('/api/admin/course_subjects', newCourseSubjects);
  ...
  const removeSubjectMutation = useMutation({
    mutationFn: async (id: string) => {
      const { error } = await apiClient.delete(`/api/admin/course_subjects/${id}`);
