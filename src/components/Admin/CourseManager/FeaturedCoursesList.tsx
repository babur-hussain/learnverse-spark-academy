...
export function FeaturedCoursesList() {
  ...
  const { data: featuredCourses, isLoading, refetch } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await apiClient.get('/api/admin/featured_courses', { params: { sort: 'order_index' } });
  ...
  const createMutation = useMutation({
    mutationFn: async (data: typeof formData) => {
      const { error } = await apiClient.post('/api/admin/featured_courses', [{
  ...
  const updateMutation = useMutation({
    mutationFn: async ({ id, data }: { id: string, data: typeof formData }) => {
      const { error } = await apiClient.put(`/api/admin/featured_courses/${id}`, {
          course_id: data.course_id,
          promotional_text: data.promotional_text || null,
          cta_text: data.cta_text,
          is_active: data.is_active,
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      const { error } = await apiClient.delete(`/api/admin/featured_courses/${id}`);
  ...
  const toggleActive = async (id: string, isActive: boolean) => {
    const { error } = await apiClient.put(`/api/admin/featured_courses/${id}`, { is_active: isActive });
  ...
  const moveOrder = async (id: string, direction: 'up' | 'down') => {
    ...
    const { error } = await apiClient.put(`/api/admin/featured_courses/${id}`, { order_index: newIndex });
