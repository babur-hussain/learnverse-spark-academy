...
export const CreateCampaign: React.FC = () => {
  ...
  const createCampaignMutation = useMutation({
    mutationFn: async (campaignData: { title: string; subject: string; content: string; status: string }) => {
      const { data: { user } } = await ({ data: { user: auth.currentUser } });
      if (!user) throw new Error('Not authenticated');

      const { data, error } = await apiClient.post('/api/admin/newsletter_campaigns', {
          ...campaignData,
          created_by: user.id
        });
  ...
  const sendCampaignMutation = useMutation({
    mutationFn: async (campaignData: { title: string; subject: string; content: string }) => {
      ...
      const { data: subscribers, error: subscribersError } = await apiClient.get(`/api/admin/newsletter_subscribers`, { params: { status: 'active' } });
      ...
      // Create campaign
      const { data: campaign, error: campaignError } = await apiClient.post('/api/admin/newsletter_campaigns', {
          ...campaignData,
          status: 'sent',
          created_by: user.id,
      ...
      const { error: logsError } = await apiClient.post('/api/admin/newsletter_campaign_logs', logs);
