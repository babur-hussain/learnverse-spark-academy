...
const GoalsManager = () => {
  ...
  const { data: goals, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await apiClient.get('/api/admin/goals');
  ...
  const mutation = useMutation({
    mutationFn: async (goal: Partial<Goal>) => {
      ...
      if (editingGoal) {
        const { error } = await apiClient.put(`/api/admin/goals/${editingGoal.id}`, {
            title: goal.title,
        ...
        const { error } = await apiClient.post('/api/admin/goals', {
            title: goal.title,
            exam_code: goal.exam_code,
            icon: goal.icon,
            is_popular: goal.is_popular,
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      const { error } = await apiClient.delete(`/api/admin/goals/${id}`);
      if (error) throw error;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['admin-goals'] });
            ...
            <TableRow key={goal.id}>
              ...
              <TableCell>
                <Switch 
                  ...
                  onCheckedChange={async (checked) => {
                    const { error } = await apiClient.put(`/api/admin/goals/${goal.id}`, { is_popular: checked });
                    if (error) {
                      toast({
                        title: 'Error',
                        description: error.message,
              ...
              <TableCell>
                <Switch 
                  checked={goal.active}
                  onCheckedChange={async (checked) => {
                    const { error } = await apiClient.put(`/api/admin/goals/${goal.id}`, { active: checked });
                    if (error) {
                      toast({
                        title: 'Error',
                        description: error.message,
