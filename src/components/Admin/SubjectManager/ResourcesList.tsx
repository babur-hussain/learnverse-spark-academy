...
const ResourcesList: React.FC<ResourcesListProps> = ({ subjectId, chapterId }) => {
  ...
  const { data: resources, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const params: any = { sort: 'order_index' };
      if (selectedSubject) params.subject_id = selectedSubject;
      if (selectedChapter) params.chapter_id = selectedChapter;
      if (resourceType) params.resource_type = resourceType;
      
      const { data, error } = await apiClient.get('/api/admin/subject_resources', { params });
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      const { error } = await apiClient.delete(`/api/admin/subject_resources/${id}`);
  ...
  const publishMutation = useMutation({
    mutationFn: async ({ id, isPublished }: { id: string; isPublished: boolean }) => {
      const { error } = await apiClient.put(`/api/admin/subject_resources/${id}`, { is_published: isPublished });
