...
export const QuizQuestionManager: React.FC<QuizQuestionManagerProps> = ({ testId }) => {
  ...
  const { data: questions = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await apiClient.get(`/api/admin/questions`, { params: { test_id: testId } });
  ...
  const addQuestionMutation = useMutation({
    mutationFn: async (questionData: typeof newQuestion) => {
      const { data, error } = await apiClient.post('/api/admin/questions', {
          test_id: testId,
  ...
  const deleteQuestionMutation = useMutation({
    mutationFn: async (questionId: string) => {
      const { error } = await apiClient.delete(`/api/admin/questions/${questionId}`);
