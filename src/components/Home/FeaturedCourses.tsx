...
const FeaturedCourses = () => {
  ...
  const { data: featuredCourses, isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      try {
        ...
        // Transform the data to flatten the course information
  ...
  if (!featuredCourses || featuredCourses.length === 0) {
    ...
    console.log('FeaturedCourses: No featured courses found, trying to get regular courses as fallback');
    
    // Query for some regular courses to show as fallback
    ...
    // Use fallback courses instead
    const coursesToDisplay = fallbackCourses;
    const sectionTitle = "Latest Courses";
    const sectionDescription = "Explore our most recent courses";
