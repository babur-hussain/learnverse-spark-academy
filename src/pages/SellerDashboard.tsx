...
const SellerDashboard = () => {
  ...
  const { data: products, isLoading: productsLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await apiClient.get(`/api/admin/products`, { params: { seller_id: user.id } });
  ...
  const { data: orders, isLoading: ordersLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await apiClient.get('/api/admin/orders')
          )
        `)
  ...
  const { data: categories } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await apiClient.get(`/api/admin/product_categories`, { params: { is_active: true } });
      if (error) throw error;
      return data;
    }
  });
  ...
  const { data: brands } = useQuery({
    queryKey: ['brands'],
    queryFn: async () => {
      const { data, error } = await apiClient.get(`/api/admin/brands`, { params: { is_active: true } });
      if (error) throw error;
      return data;
    }
  });
  ...
  const createProductMutation = useMutation({
    mutationFn: async (data: any) => {
      const { error } = await apiClient .insert({
        ...data,
        seller_id: user?.id
      });
      if (error) throw error;
  ...
  const updateProductMutation = useMutation({
    mutationFn: async ({ id, data }: { id: string; data: any }) => {
      const { error } = await apiClient .update(data).eq('id', id);
      if (error) throw error;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['seller-products'] });
  ...
  const deleteProductMutation = useMutation({
    mutationFn: async (id: string) => {
      const { error } = await apiClient .delete().eq('id', id);
      if (error) throw error;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['seller-products'] });
