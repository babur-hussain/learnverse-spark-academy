...
const TestManagement = () => {
  ...
  const { data: testStats, isLoading: isLoadingStats } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { count: totalTests } = await apiClient.get('/api/admin/tests');

      // Get published tests count
      const { count: activeTests } = await apiClient.get(`/api/admin/tests`, { params: { is_published: true } });

      // Get total submissions count
      const { count: totalSubmissions } = await apiClient.get('/api/admin/student_test_submissions');

      // Get completed submissions count
      const { count: completedSubmissions } = await apiClient.get(`/api/admin/student_test_submissions`, { params: { evaluated: true } });
  ...
  const { data: tests = [], isLoading: isLoadingTests } = useQuery({
    queryKey: ['tests-list'],
    queryFn: async () => {
      const { data: testsData, error } = await apiClient.get('/api/admin/tests');

      if (error) throw error;

      // Get questions count for each test
      const testIds = testsData?.map(t => t.id) || [];
      const { data: questionsData } = await apiClient.get('/api/admin/questions');
      ...
      const { data: submissionsData } = await apiClient.get('/api/admin/student_test_submissions');
