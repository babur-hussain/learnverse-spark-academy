...
const Home = () => {
  ...
  useEffect(() => {
    ...
    if (selectedClass) {
      ...
      if (user?.id) {
        apiClient.put('/api/users/profile', { 
            class_id: selectedClass, 
            college_id: null 
          })
          .then(() => console.log('Profile updated: class =', selectedClass, 'college = null'))
    ...
    if (selectedCollege) {
      ...
      if (user?.id) {
        apiClient.put('/api/users/profile', { 
          ...
          .then(() => console.log('Profile updated: class = null, college =', selectedCollege))
          .catch((error) => console.error('Failed to update profile:', error));
      }
    }
    ...
    if (user?.id) {
      ...
      (window as any).debugCrossDeviceSync = () => {
        ...
        apiClient.get('/api/users/profile')
          ...
          .catch((error) => console.error('❌ Database verification failed:', error));
      };
