...
const NotesManager: React.FC<NotesManagerProps> = ({ 
  ...
  const { data: chapters = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await apiClient.get('/api/admin/chapters', { params });
  ...
  const { data: noteTags = [] } = useQuery({
    queryKey: ['note_tags'],
    queryFn: async () => {
      const response = await apiClient.get('/api/admin/note_tags');
      return response.data as NoteTag[];
    }
  });
  ...
  const { data: notes = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await apiClient.get('/api/admin/note_files', { params });
      return response.data as NoteFile[];
    },
  ...
  const createNote = useMutation({
    mutationFn: async (note: {
      ...
      const uploadResult = await uploadFileToS3(note.file, filePath);

      const publicUrl = uploadResult.url || filePath;

      const response = await apiClient.post('/api/admin/note_files', [{
    ...
    onSuccess: async (data) => {
      ...
      if (selectedTags.length > 0) {
        ...
        try {
          await apiClient.post('/api/admin/note_tags', noteTagInserts);
        } catch (err: any) {
          toast({
  ...
  const updateNote = useMutation({
    mutationFn: async (note: NoteFile) => {
      const response = await apiClient.put(`/api/admin/note_files/${note.id}`, {
          title: note.title,
    ...
    onSuccess: async (data) => {
      ...
      try {
        await apiClient.delete(`/api/admin/note_tags/${noteId}`);
  ...
  const deleteNote = useMutation({
    mutationFn: async (id: string) => {
      await apiClient.delete(`/api/admin/note_files/${id}`);
      return id;
    },
    onSuccess: () => {
      toast({
  ...
  const handleDownloadNote = async (note: NoteFile) => {
    try {
      ...
      await apiClient.post('/api/admin/increment_note_download', { note_id: note.id }).catch(() => {});
    } catch (error) {
      toast({
