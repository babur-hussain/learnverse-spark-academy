...
const ChaptersList: React.FC<ChaptersListProps> = ({ subjectId, onSelectChapter }) => {
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      ...
      const { data: resources, error: resourcesError } = await apiClient.get(`/api/admin/subject_resources`, { params: { chapter_id: id } });
  ...
  const reorderMutation = useMutation({
    mutationFn: async ({ id, direction }: { id: string, direction: 'up' | 'down' }) => {
      ...
      // Swap order indexes
      const { error: error1 } = await apiClient.put(`/api/admin/chapters/${currentChapter.id}`, { order_index: targetChapter.order_index });
