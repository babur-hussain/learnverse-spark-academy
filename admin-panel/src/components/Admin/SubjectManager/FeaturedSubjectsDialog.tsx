  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      ...
      if (isEditing && featuredSubject) {
        const { error } = await apiClient /* featured_subjects */
          .update({
        ...
        try {
          ...
          const { data: maxOrderData, error: orderError } = await apiClient.get('/api/admin/featured_subjects');
        ...
        const { error } = await apiClient.post('/api/admin/featured_subjects', {
            subject_id: values.subject_id,
            is_active: values.is_active,
            promotional_text: values.promotional_text,
            cta_text: values.cta_text,
