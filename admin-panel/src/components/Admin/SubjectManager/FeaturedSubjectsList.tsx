...
export function FeaturedSubjectsList() {
  ...
  const { data: featuredSubjects, isLoading, isError, error, refetch } = useQuery({
    ...
    queryFn: async () => {
      try {
        ...
        const { data, error } = await apiClient.get('/api/admin/featured_subjects', { params: { sort: 'order_index' } });
  ...
  const toggleStatus = async (id: string, value: boolean) => {
    try {
      ...
      const { error } = await apiClient.put(`/api/admin/featured_subjects/${id}`, { is_active: value });
  ...
  const updateOrder = async (id: string, direction: 'up' | 'down') => {
    ...
    try {
      ...
      const { error: error1 } = await apiClient.put(`/api/admin/featured_subjects/${currentSubject.id}`, { order_index: targetSubject.order_index });

      if (error1) throw error1;

      const { error: error2 } = await apiClient.put(`/api/admin/featured_subjects/${targetSubject.id}`, { order_index: currentSubject.order_index });
  ...
  const handleDelete = async (id: string) => {
    try {
      ...
      const { error } = await apiClient.delete(`/api/admin/featured_subjects/${id}`);
