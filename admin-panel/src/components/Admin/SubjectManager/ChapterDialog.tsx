...
export function ChapterDialog({ 
  ...
  const { data: subjects } = useQuery({
    queryKey: ['subjects-for-chapter-dialog'],
    queryFn: async () => {
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      if (isEditing && chapter) {
        // Update chapter
        const { error } = await apiClient.put(`/api/admin/chapters/${chapter.id}`, {
            title: values.title,
        ...
        const { data: existingChapters, error: countError } = await apiClient.get(`/api/admin/chapters`, { params: { subject_id: values.subject_id } });
        ...
        // Create new chapter
        const { error } = await apiClient.post('/api/admin/chapters', {
            title: values.title,
            description: values.description,
            subject_id: values.subject_id,
