...
export function SubjectDialog({
  ...
  const fetchCategories = async () => {
    try {
      const { data, error } = await apiClient.get('/api/admin/categories');
  ...
  const fetchClasses = async () => {
    try {
      const { data, error } = await apiClient.get(`/api/admin/classes`, { params: { is_active: true } });
      if (error) throw error;
      setClasses(data || []);
    } catch (error: any) {
      toast({
  ...
  const fetchSubjectCategories = async (subjectId: string) => {
    try {
      const { data, error } = await apiClient.get(`/api/admin/content_category_mappings`, { params: { content_id: subjectId, content_type: 'subject' } });
  ...
  const fetchSubjectClass = async (subjectId: string) => {
    try {
      const { data, error } = await apiClient.get(`/api/admin/class_subjects`, { params: { subject_id: subjectId } });
  ...
  const uploadIcon = async (): Promise<string | null> => {
    ...
    try {
      ...
      const { data: buckets, error: bucketError } = await apiClient.get('/api/storage/buckets');
      ...
      if (!buckets.some(b => b.name === 'icons')) {
        ...
        const { error: createBucketError } = await apiClient.createBucket('icons', {
          public: true,
      ...
      const { data: uploadData, error: uploadError } = await uploadFileToS3(iconFile, 'uploads');
      ...
      // Get the public URL
      const { data } = { data: { publicUrl: `${import.meta.env.VITE_API_BASE_URL}/api/storage/public/${filePath}` } };
