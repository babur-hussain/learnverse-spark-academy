...
const KidsManager = () => {
  ...
  const fetchCategories = async () => {
    try {
      const { data, error } = await apiClient.get('/api/admin/kids_content_categories');
  ...
  const fetchContentItems = async () => {
    try {
      const { data, error } = await apiClient.get('/api/admin/kids_content_items', { params: { sort: '-created_at' } });
  ...
  const handleCategorySubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (selectedCategory) {
        const { error } = await apiClient.put(`/api/admin/kids_content_categories/${selectedCategory.id}`, categoryData);
        if (error) throw error;
        toast.success('Category updated successfully');
      } else {
        const { error } = await apiClient.post('/api/admin/kids_content_categories', [categoryData]);
        if (error) throw error;
  ...
  const handleContentSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (selectedContentItem) {
        const { error } = await apiClient.put(`/api/admin/kids_content_items/${selectedContentItem.id}`, contentData);
        if (error) throw error;
        toast.success('Content updated successfully');
      } else {
        const { error } = await apiClient.post('/api/admin/kids_content_items', [contentData]);
        if (error) throw error;
  ...
  const handleFileUpload = async (file: File, type: 'image' | 'video' | 'audio') => {
    try {
      ...
      const { data: urlData, error: uploadError } = await uploadFileToS3(file, filePath);
  ...
  const deleteCategory = async (id: string) => {
    try {
      const { error } = await apiClient.delete(`/api/admin/kids_content_categories/${id}`);
      if (error) throw error;
  ...
  const deleteContentItem = async (id: string) => {
    try {
      const { error } = await apiClient.delete(`/api/admin/kids_content_items/${id}`);
      if (error) throw error;
