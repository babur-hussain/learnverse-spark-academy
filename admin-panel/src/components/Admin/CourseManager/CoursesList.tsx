...
const CoursesList = () => {
  ...
  const { data: courses, isLoading, refetch } = useQuery({
    ...
    queryFn: async () => {
      try {
        ...
        const { data: coursesData, error: coursesError } = await apiClient.get('/api/admin/courses');
        ...
        const enrichedCourses = await Promise.all(
          coursesData.map(async (course) => {
            ...
            if (course.instructor_id) {
              const { data: instructorData } = await apiClient.get(`/api/admin/profiles`, { params: { id: course.instructor_id } });
                
              instructorName = instructorData?.full_name || instructorData?.username || 'No Instructor';
            }
            
            // Get subject count
            const { count: subjectCount, error: subjectError } = await apiClient.get(`/api/admin/course_subjects`, { params: { course_id: course.id } });
              
            // Get category count
            const { count: categoryCount, error: categoryError } = await apiClient.get(`/api/admin/course_categories`, { params: { course_id: course.id } });
  ...
  const toggleSubscription = async (id: string, value: boolean) => {
    const { error } = await apiClient.put(`/api/admin/courses/${id}`, { subscription_required: value });
  ...
  const togglePaid = async (id: string, value: boolean) => {
    const { error } = await apiClient.put(`/api/admin/courses/${id}`, { 
  ...
  const toggleFeatured = async (id: string, value: boolean) => {
    const { error } = await apiClient.put(`/api/admin/courses/${id}`, { featured: value });
  ...
  const handleDelete = async (id: string) => {
    ...
    const { count: subjectCount } = await apiClient.get(`/api/admin/course_subjects`, { params: { course_id: id } });
      
    const { count: categoryCount } = await apiClient.get(`/api/admin/course_categories`, { params: { course_id: id } });
    ...
    const { error } = await apiClient.delete(`/api/admin/courses/${id}`);
  ...
  const handleClone = async (course: Course) => {
    ...
    const { data, error } = await apiClient.post('/api/admin/courses', {
        title: `${title} (Copy)`,
        description,
        thumbnail_url,
        instructor_id,
