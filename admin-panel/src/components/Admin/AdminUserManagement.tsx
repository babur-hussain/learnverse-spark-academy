...
export const AdminUserManagement = () => {
  ...
  const { data: users = [], isLoading: isLoadingUsers } = useQuery({
    ...
    queryFn: async () => {
      const { data: profiles, error } = await apiClient.get('/api/admin/profiles');

      if (error) throw error;

      // Get user course enrollments count
      const userIds = profiles?.map(p => p.id) || [];
      const { data: userCourses } = await apiClient.get('/api/admin/user_courses');
  ...
  const { data: userStats, isLoading: isLoadingStats } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { count: activeStudents } = await apiClient.get(`/api/admin/profiles`, { params: { role: 'student' } });

      // Get teachers count
      const { count: teachers } = await apiClient.get(`/api/admin/profiles`, { params: { role: 'teacher' } });
