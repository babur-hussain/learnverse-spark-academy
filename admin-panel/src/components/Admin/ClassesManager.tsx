...
const ClassesManager: React.FC = () => {
  ...
  const addMutation = useMutation({
    mutationFn: async (newClass: Partial<Class>) => {
      ...
      const { data, error } = await apiClient.post('/api/admin/classes', payload);
      if (error) throw error;
      return data;
    },
    onSuccess: () => {
  ...
  const updateMutation = useMutation({
    mutationFn: async (updatedClass: Partial<Class>) => {
      const { data, error } = await apiClient.put(`/api/admin/classes/${updatedClass.id}`, updatedClass);
      if (error) throw error;
      return data;
    },
    onSuccess: () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      const { error } = await apiClient.delete(`/api/admin/classes/${id}`);
      if (error) throw error;
      return id;
    },
    onSuccess: () => {
