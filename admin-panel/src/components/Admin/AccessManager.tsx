...
const AccessManager = () => {
  ...
  const { data: notes = [] } = useQuery({
    ...
    queryFn: async () => {
      const response = await apiClient.get('/api/admin/note_files');
  ...
  const { data: noteAccess = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const response = await apiClient.get('/api/admin/note_access', { params: { order_by: 'created_at', sort: 'desc' } });
  ...
  const createNoteAccess = useMutation({
    mutationFn: async (access: {
      ...
      const { data: existingAccess } = await apiClient.get(`/api/admin/note_access`, { params: { note_id: access.note_id, role: access.role } });

      if (existingAccess && existingAccess.length > 0) {
        throw new Error('This role already has access to this note. Please edit the existing access instead.');
      }

      const response = await apiClient.post('/api/admin/note_access', [{
  ...
  const deleteNoteAccess = useMutation({
    mutationFn: async (id: string) => {
      await apiClient.delete(`/api/admin/note_access/${id}`);
      return id;
    },
    onSuccess: () => {
      toast({
