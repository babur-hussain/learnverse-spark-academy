  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      if (isEditing) {
        ...
        const { error } = await apiClient.put(`/api/admin/categories/${category.id}`, {
            name: values.name,
            slug: values.slug,
        ...
        const { data: maxOrderCategory, error: orderError } = await apiClient.get('/api/admin/categories');
        ...
        const { data: existingSlug } = await apiClient.get(`/api/admin/categories`, { params: { slug: values.slug } });
        ...
        const { error, data } = await apiClient.post('/api/admin/categories', {
            name: values.name,
            slug: values.slug,
            description: values.description,
            icon: values.icon,
