...
export function FeaturedCategoryDialog({ 
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      if (isEditing) {
        const { error } = await apiClient.put(`/api/admin/featured_categories/${featuredCategory.id}`, {
        ...
        const { data: maxOrderCategory, error: orderError } = await apiClient.get('/api/admin/featured_categories');

        const nextOrderIndex = maxOrderCategory ? maxOrderCategory.order_index + 1 : 1;

        const { error } = await apiClient.post('/api/admin/featured_categories', {
            category_id: values.category_id,
            is_active: values.is_active,
            promotional_text: values.promotional_text,
            cta_text: values.cta_text,
