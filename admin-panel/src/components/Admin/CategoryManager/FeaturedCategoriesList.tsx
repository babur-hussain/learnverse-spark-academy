...
export function FeaturedCategoriesList() {
  ...
  const { data: featuredCategories, isLoading, refetch } = useQuery({
    ...
    queryFn: async () => {
      const response = await apiClient.get('/api/admin/featured_categories', { params: { order_by: 'order_index', sort: 'asc' } });
  ...
  const toggleStatus = async (id: string, value: boolean) => {
    try {
      await apiClient.put(`/api/admin/featured_categories/${id}`, { is_active: value });
      toast({
        title: "Success",
  ...
  const updateOrder = async (id: string, direction: 'up' | 'down') => {
    ...
    try {
      await apiClient.put(`/api/admin/featured_categories/${id}`, { order_index: featuredCategories[newIndex].order_index });
      await apiClient.put(`/api/admin/featured_categories/${featuredCategories[newIndex].id}`, { order_index: featuredCategories[currentIndex].order_index });
      refetch();
    } catch (err) {
  ...
  const handleDelete = async (id: string) => {
    try {
      await apiClient.delete(`/api/admin/featured_categories/${id}`);
      toast({
        title: "Success",
