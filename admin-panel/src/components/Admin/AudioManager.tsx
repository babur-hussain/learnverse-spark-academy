...
const AudioManager: React.FC<AudioManagerProps> = ({ selectedCourse, onSelectCourse }) => {
  ...
  const { data: courses = [] } = useQuery({
    ...
    queryFn: async () => {
      if (newAudio.class_id) {
        const { data, error } = await apiClient.get('/api/admin/class_subjects', { params: { class_id: newAudio.class_id } });
        if (error) throw error;
        return (data || []).map(cs => cs.subjects);
      } else if (newAudio.college_id) {
        const { data, error } = await apiClient.get('/api/admin/college_courses', { params: { college_id: newAudio.college_id } });
        if (error) throw error;
  ...
  const { data: audioFiles = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      let query = apiClient.get('/api/admin/audio_files');
      if (selectedCourse) {
  ...
  const { data: classes = [] } = useQuery({
    queryKey: ['classes'],
    queryFn: async () => {
      const { data, error } = await apiClient.get('/api/admin/classes');
      if (error) throw error;
  ...
  const createAudio = useMutation({
    mutationFn: async (audio: {
      ...
      const { data: urlData, error: uploadError } = await uploadFileToS3(audio.file, filePath);
      if (uploadError) throw uploadError;
      const { data, error } = await apiClient.post('/api/admin/audio_files', [{
  ...
  const updateAudio = useMutation({
    mutationFn: async (audio: AudioFile) => {
      const { data, error } = await apiClient /* audio_files */
        .update({
          title: audio.title,
  ...
  const deleteAudio = useMutation({
    mutationFn: async (id: string) => {
      const { error } = await apiClient.delete(`/api/admin/audio_files/${id}`);
      if (error) throw error;
      return id;
    },
    onSuccess: () => {
